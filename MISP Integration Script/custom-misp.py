...
# Check if the alert contains the "data" key
if "data" not in alert:
    sys.exit("No 'data' field in alert, skipping integration.")

# New Alert Output if MISP Alert or Error calling the API
alert_output = {}
# MISP Server Base URL
misp_base_url = "[YOUR-IP]"
...
if event_source == "windows":
    if event_type == "sysmon_event1":
        ...
        event_type == "sysmon_event3"
        and alert["data"]["win"]["eventdata"]["destinationIsIpv6"] == "true"
    ):
        sys.exit()
    ...
    try:
        ...
        if misp_api_response["response"]["Attribute"]:
            ...
            # Initialize 'source' dictionary before use
            alert_output["misp"]["source"] = {}
            alert_output["misp"]["source"]["description"] = alert["rule"]["description"]
            # Add command field to trigger active response
            alert_output["command"] = "add"
            send_event(alert_output, alert["agent"])
            ...
            if ipaddress.ip_address(dst_ip).is_global:
                ...
                try:
                    ...
                    if misp_api_response["response"]["Attribute"]:
                        ...
                        # Initialize 'source' dictionary before use
                        alert_output["misp"]["source"] = {}
                        # For Linux, rule description might not be available, add a placeholder if needed
                        alert_output["misp"]["source"]["description"] = alert["rule"].get("description", "No description")
                        # Add command field to trigger active response
                        alert_output["command"] = "add"
                        send_event(alert_output, alert["agent"])
            else:
